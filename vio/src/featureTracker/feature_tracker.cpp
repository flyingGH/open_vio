...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (prev_pts.size() > 0)
    {
        ...
        PRINT_INFO("temporal optical flow costs: %fms\n", t_o.toc());
        //PRINT_INFO("track cnt %d\n", (int)ids.size());
    }
    ...
    if (1)
    {
        ...
        //rejectWithF();
        PRINT_DEBUG("set mask begins\n");
        TicToc t_m;
        // 特征点画个圈存mask图，同时特征点集合按跟踪次数从大到小重排序
        setMask();
        PRINT_DEBUG("set mask costs %fms\n", t_m.toc());

        PRINT_DEBUG("detect feature begins\n");
        TicToc t_t;
        ...
        {
            if(mask.empty())
                PRINT_DEBUG("mask is empty\n")
            if (mask.type() != CV_8UC1)
                PRINT_DEBUG("mask type wrong\n")
            // 精确角点提取
            cv::goodFeaturesToTrack(cur_img, n_pts, MAX_CNT - cur_pts.size(), 0.01, MIN_DIST, mask);
        }
        else
            n_pts.clear();
        PRINT_DEBUG("detect feature costs: %f ms\n", t_t.toc());
        ...
        //PRINT_INFO("feature cnt after add %d\n", (int)ids.size());
    }
    ...
    if(!_img1.empty() && stereo_cam)
    {
        ...
        if(!cur_pts.empty())
        {
            //PRINT_INFO("stereo image; track feature on right image\n");
    ...
    //PRINT_INFO("feature track whole time %f\n", t_r.toc());
    return featureFrame;
}
...
void FeatureTracker::rejectWithF()
{
    if (cur_pts.size() >= 8)
    {
        ...
        reduceVector(ids, status);
        reduceVector(track_cnt, status);
        PRINT_DEBUG("FM ransac: %d -> %lu: %f\n", size_a, cur_pts.size(), 1.0 * cur_pts.size() / size_a);
        PRINT_INFO("FM ransac costs: %fms\n", t_f.toc());
    }
}
...
void FeatureTracker::readIntrinsicParameter(const vector<string> &calib_file) {
    for (size_t i = 0; i < calib_file.size(); i++) {
        PRINT_INFO("reading paramerter of camera %s\n", calib_file[i].c_str());
...
void FeatureTracker::showUndistortion(const string &name)
{
    ...
    for (int i = 0; i < int(undistortedp.size()); i++)
    {
        ...
        {
            //PRINT_ERROR("(%f %f) -> (%f %f)", distortedp[i].y, distortedp[i].x, pp.at<float>(1, 0), pp.at<float>(0, 0));
        }
    }
...
void FeatureTracker::setPrediction(map<int, Eigen::Vector3d> &predictPts)
{
    ...
    for (size_t i = 0; i < ids.size(); i++)
    {
        //PRINT_INFO("prevLeftId size %d prevLeftPts size %d\n",(int)prevLeftIds.size(), (int)prevLeftPts.size());
        int id = ids[i];
