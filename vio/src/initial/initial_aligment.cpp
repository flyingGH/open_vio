...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    //  PRINT_WARNING_STREAM("gyroscope bias initial calibration " << delta_bg.transpose());
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    PRINT_DEBUG("estimated scale: %f\n", s);
    g = x.segment<3>(n_state - 4);
    // PRINT_DEBUG_STREAM(" result g     " << g.norm() << " " << g.transpose());
    ...
    s = (x.tail<1>())(0) / 100.0;
    (x.tail<1>())(0) = s;
    // PRINT_DEBUG_STREAM(" refine     " << g.norm() << " " << g.transpose());
    if(s < 0.0 )
        return false;   
    else
        return true;
