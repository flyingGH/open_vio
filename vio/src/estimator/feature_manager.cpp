...
{
    PRINT_DEBUG("input feature: %d\n", (int)image.size());
    PRINT_DEBUG("num of feature: %d\n", getFeatureCount());
    ...
    if (parallax_num == 0) {
        ...
        PRINT_DEBUG("parallax_sum: %lf, parallax_num: %d\n", parallax_sum, parallax_num);
        PRINT_DEBUG("current parallax: %lf\n    ", parallax_sum / parallax_num * FOCAL_LENGTH);
                                      ...
                                      vector<cv::Point2f> &pts2D, vector<cv::Point3f> &pts3D)
{
    ...
    //PRINT_INFO("pnp size %d \n",(int)pts2D.size() );
    // 特征点太少
    if (int(pts2D.size()) < 4)
    {
        PRINT_INFO("feature tracking not enough, please slowly move you device! \n");
        return false;
    }
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        if(STEREO && it_per_id.feature_per_frame[0].is_stereo)
        {
            ...
            PRINT_INFO("stereo %d pts: %f %f %f gt: %f %f %f \n",it_per_id.feature_id, point3d.x(), point3d.y(), point3d.z(),
        ...
        else if(it_per_id.feature_per_frame.size() > 1)
        {
            ...
            /*
            Vector3d ptsGt = pts_gt[it_per_id.feature_id];
            PRINT_INFO("motion  %d pts: %f %f %f gt: %f %f %f \n",it_per_id.feature_id, point3d.x(), point3d.y(), point3d.z(),
                                                            ptsGt.x(), ptsGt.y(), ptsGt.z());
            */
            continue;
        }
