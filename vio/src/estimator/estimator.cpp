...
void Estimator::setParameter()
{
    ...
    // imagesBinFile.open(IMAGE_BIN_FOLDER, std::ios::trunc | std::ios::binary);
    imagesBinFile.open(IMAGE_BIN_FOLDER, fstream::out);
    if(!imagesBinFile.is_open()) {
        PRINT_ERROR("%s is not open\n", IMAGE_BIN_FOLDER)
    }
    vioFile.open(VINS_RESULT_PATH, fstream::out);
    if(!vioFile.is_open()) {
        PRINT_ERROR("%s is not open\n", VINS_RESULT_PATH)
    }
    keyframe_cnt = 0;
...
void Estimator::changeSensorType(int use_imu, int use_stereo)
{
    ...
    if(!use_imu && !use_stereo) {
        PRINT_INFO("at least use two sensors! \n");
    } else {
        if(USE_IMU != use_imu) {
            USE_IMU = use_imu;
            if(USE_IMU) {
        ...
        STEREO = use_stereo;
        PRINT_INFO("use imu %d use stereo %d\n", USE_IMU, STEREO);
    }
    mProcess.unlock();
    if(restart)
    {
...
void Estimator::inputImage(double t, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    //PRINT_INFO("featureTracker time: %f\n", featureTrackerTime.toc());
...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration, const Vector3d &angularVelocity)
{
    ...
    //PRINT_INFO("input imu with time %f \n", t);
    mBuf.unlock();
                                ...
                                vector<pair<double, Eigen::Vector3d>> &gyrVector)
{
    if(accBuf.empty())
    {
        PRINT_INFO("not receive imu\n");
        return false;
    }
    //PRINT_INFO("get imu from %f %f\n", t0, t1);
    //PRINT_INFO("imu fornt time %f   imu end time %f\n", accBuf.front().first, accBuf.back().first);
...
void Estimator::processMeasurements() {
    while (1) {
        //PRINT_INFO("process measurments\n");
        ...
        if(!featureBuf.empty()) {
            ...
            if(USE_IMU) {
                // 第一帧 IMU 姿态初始化
                // 用初始时刻加速度方向对齐重力加速度方向，得到一个旋转，使得初始 IMU 的 z 轴指向重力加速度方向
                if(!initFirstPoseFlag) {
                    initFirstIMUPose(accVector);
                }

                // IMU积分
                // 用前一图像帧位姿，前一图像帧与当前图像帧之间的 IMU 数据，积分计算得到当前图像帧位姿
                // Rs，Ps，Vs
            ...
            if (solver_flag == SolverFlag::NON_LINEAR) {
                ...
                Quaterniond tmp_Q; 
                tmp_Q = Quaterniond(this->Rs[WINDOW_SIZE]);
                Eigen::Vector3d tmp_ps = this->Ps[WINDOW_SIZE];
                Eigen::Vector3d tmp_vs = this->Vs[WINDOW_SIZE];
                ...
                if (marginalization_flag == 0) {
                    keyframe_cnt++;
                    // 只保存了左目的信息
                    Eigen::Vector3d cam0_p = Ps[WINDOW_SIZE] + Rs[WINDOW_SIZE] * tic[0];
                    Quaterniond cam0_q = Quaterniond(Rs[WINDOW_SIZE] * ric[0]);

                    ostringstream oss;
                    oss << keyframe_cnt;
                    std::string image_name = oss.str() + ".jpg";
                    cv::imwrite(IMAGE_FOLDER + image_name, last_img0);

                    imagesBinFile.setf(ios::fixed, ios::floatfield);
                    imagesBinFile << image_name << ",";
                    imagesBinFile.precision(9);
                    imagesBinFile << dStamp << ",";
                    imagesBinFile.precision(5);
                    imagesBinFile << cam0_p.x() << "," << cam0_p.y() << "," << cam0_p.z() << ","
                        << cam0_q.w()  << "," << cam0_q.x()  << "," << cam0_q.y()  << "," << cam0_q.z() << std::endl;
                }
            }
...
{
    PRINT_INFO("init first imu pose\n");
...
{
    PRINT_DEBUG("new image coming ------------------------------------------\n");
    PRINT_DEBUG("Adding feature points %lu\n", image.size());
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td)) {
        ...
        //PRINT_INFO("non-keyframe\n");
    }

    PRINT_DEBUG("%s\n", marginalization_flag ? "Non-keyframe" : "Keyframe");
    PRINT_DEBUG("Solving %d\n", frame_count);
    PRINT_DEBUG("number of feature: %d\n", f_manager.getFeatureCount());
    ...
    {
        PRINT_INFO("calibrating extrinsic param, rotation movement is needed\n");
        if (frame_count != 0)
        {
            // 提取前一帧与当前帧的匹配点
            ...
            */
            ...
            {
                PRINT_WARNING("initial extrinsic rotation calib success\n");
                // //  PRINT_WARNING_STREAM("initial extrinsic rotation: " << endl << calib_ric);
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
    ...
    if (solver_flag == INITIAL) {
        ...
        if (!STEREO && USE_IMU)
        {
            ...
            if (frame_count == WINDOW_SIZE)
            {
                ...
                if(result) {
                    ...
                    slideWindow();
                    PRINT_INFO("Initialization finish!\n");
                }
                else
                    // 移动滑窗，更新特征点的观测帧集合、观测帧索引（在滑窗中的位置）、首帧观测帧和深度值，删除没有观测帧的特征点
                    slideWindow();
        ...
        */
        ...
        PRINT_INFO("solver costs: %fms\n", t_solve.toc());

        // 失败检测，失败了重置系统
        if (failureDetection()) {
            PRINT_WARNING("failure detection!\n");
            failure_occur = 1;
            clearState();
            setParameter();
...
bool Estimator::initialStructure()
{
    ...
    // 计算滑窗内IMU加速度的标准差，用于判断移动快慢，为什么不用旋转或者平移量判断 todo
    {
        ...
        //PRINT_WARNING("IMU variation %f!\n", var);
        if(var < 0.25) {
            PRINT_INFO("IMU excitation not enouth!\n");
            //return false;
        }
    }
    // global sfm
    ...
    */
    if (!relativePose(relative_R, relative_T, l)) {
        PRINT_INFO("Not enough features or parallax; Move device around\n");
        return false;
    }
    /**
    ...
    */
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if(pts_3_vector.size() < 6) {
            ...
            PRINT_DEBUG("Not enough points for solve pnp !\n");
            return false;
        }
        // 3d-2d Pnp求解位姿
        if (! cv::solvePnP(pts_3_vector, pts_2_vector, K, D, rvec, t, 1)) {
            PRINT_DEBUG("solve pnp fail!\n");
            return false;
        }
        cv::Rodrigues(rvec, r);
    ...
    if (visualInitialAlign()) {
        ...
        PRINT_INFO("misalign visual structure with IMU\n");
        return false;
    }
}
...
bool Estimator::visualInitialAlign()
{
    ...
    // PRINT_DEBUG_STREAM("g0     " << g.transpose());
    // PRINT_DEBUG_STREAM("my R0  " << Utility::R2ypr(Rs[0]).transpose()); 
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            {
                l = i;
                PRINT_DEBUG("average_parallax %f choose l %d and newest frame to triangulate the whole structure\n", average_parallax * 460, l);
                return true;
            }
        }
    }
...
void Estimator::double2vector()
{
    ...
    if(USE_IMU)
    {
        ...
        {
            PRINT_DEBUG("euler singular point!\n");
            // 计算旋转位姿变换
...
bool Estimator::failureDetection()
{
    ...
    {
        PRINT_INFO(" little feature %d\n", f_manager.last_track_num);
        //return true;
    }
    // 偏置太大
    if (Bas[WINDOW_SIZE].norm() > 2.5)
    {
        PRINT_INFO(" big IMU acc bias estimation %f\n", Bas[WINDOW_SIZE].norm());
        return true;
    }
    // 偏置太大
    if (Bgs[WINDOW_SIZE].norm() > 1.0)
    {
        PRINT_INFO(" big IMU gyr bias estimation %f\n", Bgs[WINDOW_SIZE].norm());
        return true;
    }
    /*
    if (tic(0) > 1)
    {
        PRINT_INFO(" big extri param estimation %d\n", tic(0) > 1);
        return true;
    }
    */
    // 优化前后当前帧位置相差过大
    ...
    {
        //PRINT_INFO(" big translation\n");
        //return true;
    }
    if (abs(tmp_P.z() - last_P.z()) > 1)
    {
        //PRINT_INFO(" big z translation\n");
        //return true; 
    }
    // 当前帧位姿优化前后角度相差过大
...
void Estimator::optimization()
{
    ...
    */
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if ((ESTIMATE_EXTRINSIC && frame_count == WINDOW_SIZE && Vs[0].norm() > 0.2) || openExEstimation) {
            //PRINT_INFO("estimate extinsic param\n");
            openExEstimation = 1;
        } else {
            //PRINT_INFO("fix extinsic param\n");
            // 不估计外参的时候，固定外参
    ...
    */
    ...
    PRINT_DEBUG("visual measurement count: %d\n", f_m_cnt);
    //PRINT_INFO("prepare for ceres: %f \n", t_prepare.toc());
    ...
    */
    ...
    PRINT_DEBUG("Iterations : %d\n", static_cast<int>(summary.iterations.size()));
    //PRINT_INFO("solver costs: %f \n", t_solver.toc());

    // 更新优化后的参数，包括位姿、速度、偏置、外参、特征点逆深度、相机与IMU时差
    double2vector();
    //PRINT_INFO("frame_count: %d \n", frame_count);
    ...
    */
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        PRINT_DEBUG("pre marginalization %f ms\n", t_pre_margin.toc());
        
        // 执行marg边缘化
        TicToc t_margin;
        marginalization_info->marginalize();
        PRINT_DEBUG("marginalization %f ms\n", t_margin.toc());
    ...
    else
    {
            ...
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            TicToc t_pre_margin;
            PRINT_DEBUG("begin marginalization\n");
            marginalization_info->preMarginalize();
            PRINT_DEBUG("end pre marginalization, %f ms\n", t_pre_margin.toc());

            TicToc t_margin;
            PRINT_DEBUG("begin marginalization\n");
            marginalization_info->marginalize();
            PRINT_DEBUG("end marginalization, %f ms\n", t_margin.toc());
    ...
    //PRINT_INFO("whole marginalization costs: %f \n", t_whole_marginalization.toc());
    //PRINT_INFO("whole time for ceres: %f \n", t_whole.toc());
}
...
{
    //PRINT_INFO("predict pts in next frame\n");
    if(frame_count < 2)
        return;
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        if(it_per_id.estimated_depth > 0)
        {
            ...
            //PRINT_INFO("cur frame index  %d last frame index %d\n", frame_count, lastIndex);
    ...
    //PRINT_INFO("estimator output %d predict pts\n",(int)predictPts.size());
}
