...
void ResidualBlockInfo::Evaluate()
{
    ...
    if (loss_function)
    {
        ...
        //PRINT_INFO("sq_norm: %f, rho[0]: %f, rho[1]: %f, rho[2]: %f\n", sq_norm, rho[0], rho[1], rho[2]);
...
{
    //PRINT_WARNING("release marginlizationinfo\n");
...
void MarginalizationInfo::marginalize()
{
    ...
    // 保留的变量总数
    n = pos - m;
    //PRINT_INFO("marginalization, pos: %d, m: %d, n: %d, size: %d\n", pos, m, n, (int)parameter_block_idx.size());
    if(m == 0)
    {
        valid = false;
        PRINT_INFO("unstable tracking...\n");
        return;
    }
    ...
    PRINT_INFO("summing up costs %f ms\n", t_summing.toc());
    */
    //multi thread
    ...
    for (int i = 0; i < NUM_THREADS; i++)
    {
        ...
        if (ret != 0) {
            PRINT_WARNING("pthread_create error\n");
            break;
        }
    }
    ...
    //PRINT_DEBUG("thread summing up costs %f ms\n", t_thread_summing.toc());
    //PRINT_INFO("A diff %f , b diff %f \n", (A - tmp_A).sum(), (b - tmp_b).sum());
...
{
    //PRINT_INFO("internal addr,%d, %d\n", (int)parameter_block_sizes().size(), num_residuals());
    //for (int i = 0; i < static_cast<int>(keep_block_size.size()); i++)
    //{
    //    //PRINT_INFO("unsigned %x\n", reinterpret_cast<unsigned long>(parameters[i]));
    //    //PRINT_INFO("signed %x\n", reinterpret_cast<long>(parameters[i]));
    //PRINT_INFO("jacobian %x\n", reinterpret_cast<long>(jacobians));
    //PRINT_INFO("residual %x\n", reinterpret_cast<long>(residuals));
    //}
    // 上一次Marg操作之后保留的变量个数
